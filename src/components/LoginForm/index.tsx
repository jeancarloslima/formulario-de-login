  ...
  return (
    <div className="w-full max-w-100 md:max-w-250 md:min-h-140 md:flex p-6 md:p-4 bg-zinc-800 rounded-2xl shadow-2xl">
        <div className="hidden relative md:w-[50%] md:flex md:flex-col md:items-center md:justify-between">
          <img src={imagemFundo} alt="" className="w-full h-full absolute top-0 left-0 rounded-xl z-0" />
          <h2 className="font-bold text-3xl text-fuchsia-50 pt-8 z-10">AMU</h2>
          <p className="text-xl font-semibold text-fuchsia-50 text-center pb-8 z-10">Capturando Momentos, <br />Criando Memórias</p>
        </div>
        ...
        <div className="flex md:w-[50%] md:items-center md:justify-center">
          <div className="w-full md:max-w-70 flex flex-col gap-4 text-indigo-50">
            ...
            <span>Nem tem conta ainda? <Link to="/auth/register" className="text-rose-400 underline">Registrar</Link></span>
            ...
            <form onSubmit={handleSubmit(handleRegisterUser)} className="flex flex-col gap-4 mt-3">
                ...
                <button type="submit" className="w-full py-2 bg-rose-700 rounded-lg hover:bg-rose-500 hover:cursor-pointer duration-200">Entrar</button>
            </form>
            </div>
        </div>
    </div>
